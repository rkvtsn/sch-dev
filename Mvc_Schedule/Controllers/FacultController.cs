...
namespace Mvc_Schedule.Controllers
{
    ...
    public class FacultController : Controller
    {
        ...
        public ActionResult Edit(int id)
        {
            ...
            var model = new FacultEdit { FacultInstance = facult };
            ViewBag.IsTemplateExist = System.IO.File.Exists(ExcelTemplate.Path(id));
            return View(model);
        }
        ...
        public ActionResult Edit(FacultEdit model)
        {
            if (ModelState.IsValid)
            {
                _db.Facults.Edit(ExcelTemplate.Update(model.Template, model.FacultInstance));
                _db.SaveChanges();
                return RedirectToAction("Index");
            }
            return View(model);
